...
OPTIONAL:
  --original-dir PATH       Custom path to C/original documentation directory (default: uses config)
  --generated-dir PATH      Custom path to generated/sketch documentation directory (default: uses config)
...
EXAMPLES:
  ...
  # With custom paths
  $0 --repo-name myrepo --original-dir /path/to/c_docs --generated-dir /path/to/sketch_docs
...
if [[ -n "$ORIGINAL_DIR" && -n "$GENERATED_DIR" ]]; then
    # Use custom directories
    DATA_DIR="$PROJECT_ROOT/data/$REPO_NAME"  # Still need for output
    mkdir -p "$DATA_DIR"
    ORIGINAL_DOCS_DIR="$ORIGINAL_DIR"
    GENERATED_DOCS_DIR="$GENERATED_DIR"
else
    ...
    ORIGINAL_DOCS_DIR="$DATA_DIR/original"
    GENERATED_DOCS_DIR="$DATA_DIR/${REFERENCE:-deepwiki}"
fi

# Validate directories exist
if [[ ! -d "$ORIGINAL_DOCS_DIR" ]]; then
    print_error "Original docs directory not found: $ORIGINAL_DOCS_DIR"
    exit 1
fi

if [[ ! -d "$GENERATED_DOCS_DIR" ]]; then
    print_error "Generated docs directory not found: $GENERATED_DOCS_DIR"
    exit 1
fi

# Check if docs tree exists
if [[ ! -f "$GENERATED_DOCS_DIR/docs_tree.json" ]]; then
    print_error "Documentation tree not found: $GENERATED_DOCS_DIR/docs_tree.json"
...
print_status "Original docs: $ORIGINAL_DOCS_DIR"
print_status "Generated docs: $GENERATED_DOCS_DIR"
print_status "Output directory: $DATA_DIR"
...
if [[ "$SKIP_EVALUATION" == false ]]; then
    ...
    while [[ -n "$remaining_models" ]]; do
        ...
        eval_cmd="python judge/judge.py --repo-name \"$REPO_NAME\" --model \"$model\" --batch-size $BATCH_SIZE --max-retries $MAX_RETRIES"
        
        # Add custom paths if provided
        if [[ -n "$ORIGINAL_DIR" ]]; then
            eval_cmd="$eval_cmd --original-dir \"$ORIGINAL_DOCS_DIR\""
        fi
        if [[ -n "$GENERATED_DIR" ]]; then
            eval_cmd="$eval_cmd --generated-dir \"$GENERATED_DOCS_DIR\""
        else
            # Use reference name for default path structure
            eval_cmd="$eval_cmd --reference \"${REFERENCE:-deepwiki}\""
        fi
...
if [[ "$SKIP_COMBINATION" == false ]] && [[ $model_count -gt 1 ]]; then
    ...
    if [[ -n "$GENERATED_DOCS_DIR" ]]; then
        combine_cmd="$combine_cmd --generated-dir \"$GENERATED_DOCS_DIR\""
    fi
...
if [[ "$VISUALIZE" == true ]]; then
    ...
    if [[ -f "judge/visualize_evaluation.py" ]]; then
        visualize_cmd="python judge/visualize_evaluation.py --repo-name \"$REPO_NAME\""
        
        if [[ -n "$GENERATED_DOCS_DIR" ]]; then
            visualize_cmd="$visualize_cmd --generated-dir \"$GENERATED_DOCS_DIR\""
        else
            visualize_cmd="$visualize_cmd --reference \"${REFERENCE:-deepwiki}\""
        fi
        
        print_status "Running: $visualize_cmd"
        eval $visualize_cmd
