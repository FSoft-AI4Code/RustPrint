...
def convert_to_dict(obj):
    ...
    if isinstance(obj, DocPage):
        ...
        # Sanitize metadata path if it exists
        if "metadata" in result and "path" in result["metadata"]:
            metadata_path = result["metadata"]["path"]
            if isinstance(metadata_path, str):
                result["metadata"]["path"] = os.path.normpath(metadata_path) if metadata_path else None
        return result
...
def parse_docs_directory(path: str, project_name: str = None, output_dir: str = None) -> tuple[DocPage, Dict]:
    ...
    docs_root = normalized_path  # Store the root docs path for SVG resolution
    ...
    def process_directory(dir_path: str, current_page: DocPage) -> None:
        ...
        try:
            ...
            print(f"Warning: Could not normalize directory path {dir_path}: {e}")
            return
        
        # Get all items in directory and sort them
        try:
            dir_items = os.listdir(normalized_dir)
        ...
        for item in dir_items:
            ...
            try:
                ...
                if os.path.isfile(normalized_item_path) and (item.endswith('.md') or item.endswith('.mdx')):
                    files.append((item, normalized_item_path))
                elif os.path.isdir(normalized_item_path) and not item.startswith('.'):
                    directories.append((item, normalized_item_path))
            except (ValueError, OSError) as e:
                print(f"Warning: Could not normalize item path {item_path}: {e}")
                continue
    ...
    root_page = DocPage(
        ...
        content={},
        metadata={"type": "root", "path": normalized_path},
        subpages=[]
    )
    
    # Process the root directory
    process_directory(normalized_path, root_page)
...
if __name__ == "__main__":
    ...
    parser.add_argument('--docs-path', required=False, help='Path to the documentation directory')
    parser.add_argument('--output-dir', required=False, help='Output directory for parsed documentation')
    args = parser.parse_args()
    
    project_name = args.repo_name
    
    if args.docs_path and args.output_dir:
        ...
        try:
            ...
            print("Error: Either provide --docs-path and --output-dir, or ensure config module is available")
            exit(1)
