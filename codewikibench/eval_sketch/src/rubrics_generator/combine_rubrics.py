...
def parse_args():
    ...
    parser.add_argument("--model", required=True, help="Model to use for combining rubrics")
...
def fallback_simple_merge(all_rubrics: List[List[Dict]]) -> List[Dict]:
    ...
    for item in all_items:
        name = item.get("requirements", item.get("name", "")).lower().strip()
...
def load_rubrics_files(repo_name: str, data_dir: str = None) -> List[Dict]:
    """Load all rubrics files for the repository"""
    if data_dir:
        base_path = os.path.join(data_dir, "rubrics")
    else:
...
async def main():
    ...
    if args.data_dir:
        base_path = os.path.join(args.data_dir, "rubrics")
    else:
    ...
    all_rubrics = load_rubrics_files(args.repo_name, args.data_dir)
    ...
    print(f"Method used: Semantic LLM Analysis ({args.model})")
