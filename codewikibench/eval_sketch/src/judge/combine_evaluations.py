...
def parse_args():
    ...
    parser.add_argument("--generated-dir", help="Custom path to generated/sketch documentation directory (default: uses config)")
...
def load_evaluation_files(repo_name: str, reference: str, generated_dir: str = None) -> List[Dict]:
    """Load all evaluation files matching the pattern"""
    if generated_dir:
        base_path = os.path.join(generated_dir, "evaluation_results")
    else:
...
def main():
    ...
    evaluations = load_evaluation_files(args.repo_name, args.reference, args.generated_dir)
    ...
    if len(evaluations) == 1:
        print("Only 1 evaluation file found. Creating a copy as combined result.")
        combined_rubrics = evaluations[0]
        
        # Add metadata
        combination_metadata = {
            "combination_method": "single_model",
            "num_evaluations_combined": 1,
            "note": "Only one evaluation file was available, so no combination was performed"
        }
        
        # Save combined results
        if args.generated_dir:
            base_path = os.path.join(args.generated_dir, "evaluation_results")
        else:
