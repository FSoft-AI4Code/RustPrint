...
def parse_args():
    ...
    parser.add_argument("--reference", help="Name of the folder that contains the reference documentation (for default paths)")
    parser.add_argument("--original-dir", help="Custom path to original/C documentation directory")
    parser.add_argument("--generated-dir", help="Custom path to generated/sketch documentation directory")
...
IMPORTANT: This evaluation must be language-flexible and focus on FEATURES and FUNCTIONALITY, not language-specific implementation details.
- Evaluate based on **WHAT** features/capabilities are described, not **HOW** they are implemented in a specific language
- Focus on functional descriptions, architectural concepts, and feature coverage
- Consider equivalent concepts across different languages (e.g., "memory management" in C vs "ownership/borrowing" in Rust)
- Do NOT penalize documentation for using different terminology or language-specific constructs if the underlying feature/capability is documented
- Documentation in different languages (e.g., C vs Rust) should be evaluated fairly based on feature coverage, not syntax or language constructs
...
4. Focus on **functional equivalence** - if a feature/capability is described in different terms or using different language constructs, still count it as documented
...
```json
{
  ...
  "score": 0 or 1,
  "reasoning": "Brief explanation of why this score was assigned (focusing on feature coverage, not language specifics)",
...
IMPORTANT: Focus on FEATURES and FUNCTIONALITY, not language-specific implementation details.
- Evaluate based on whether the underlying feature/capability is documented
- Consider equivalent concepts across different languages
- Do NOT penalize for different terminology or language constructs if the feature is covered
...
""".strip()
            ...
            if "429" in error_msg or "rate limit" in error_msg.lower():
                tqdm.write("!! Rate limit — waiting 60s before retry !!")
                await asyncio.sleep(60)
            elif any(code in error_msg for code in ["2005", "2019", "500", "502", "503", "504"]):
                tqdm.write("!! Transient provider error — waiting 10s before retry !!")
            ...
            try:
                tqdm.write(f"!! Retrying {leaf['requirement'][:50]} !!")
                if agent is None:
                    final_output = await run_llm_natively(model, messages=[{"role": "system", "content": system_prompt}, {"role": "user", "content": prompt}])
                else:
                    result = await run_agent_with_retry(agent, prompt, deps=deps)
                ...
                json_end = final_output.rfind('}') + 1
                if json_start != -1 and json_end > json_start:
                    evaluation = json.loads(final_output[json_start:json_end])
                    return leaf['path'], {
                        "score": evaluation.get("score", 0),
                        "reasoning": evaluation.get("reasoning", "No reasoning provided"),
                        "evidence": evaluation.get("evidence", "No evidence provided"),
                ...
                tqdm.write(f"!! Retry also failed: {str(retry_e)[:100]} !!")

            return leaf['path'], {
                "score": 0,
                "reasoning": f"[EVALUATION ERROR — retry failed]: {error_msg}",
                "evidence": f"Full error: {error_msg}",
                "tokens": {"input": 0, "output": 0}
            }
...
async def run(args):
    if args.original_dir and args.generated_dir:
        ...
        base_path = os.path.dirname(generated_path)  # Use parent of generated for output
        docs_path = generated_path
        docs_tree_path = os.path.join(docs_path, "docs_tree.json")
        rubrics_dir = os.path.join(original_path, "rubrics")
        output_dir = generated_path
    else:
        # Use default config-based paths
        base_path = config.get_data_path(args.repo_name)
        docs_path = os.path.join(base_path, args.reference or "deepwiki")
        docs_tree_path = os.path.join(docs_path, "docs_tree.json")
        rubrics_dir = os.path.join(base_path, "rubrics")
    ...
    if not os.path.exists(docs_tree_path):
        print(f"Documentation tree not found: {docs_tree_path}")
        return
    ...
    rubrics_file = args.rubrics_file or os.path.join(rubrics_dir, "combined_rubrics.json")
    ...
    evaluation_folder = os.path.join(output_dir, "evaluation_results")
