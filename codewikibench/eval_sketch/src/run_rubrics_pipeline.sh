...
OPTIONAL:
  --data-dir PATH           Custom data directory path (default: uses config)
...
EXAMPLES:
  ...
  $0 --repo-name myrepo --data-dir /path/to/c_docs/myrepo
...
# Check if docs tree exists (assuming it's in the 'original' subfolder or at root)
if [[ -f "$DATA_DIR/original/docs_tree.json" ]]; then
    DOCS_TREE="$DATA_DIR/original/docs_tree.json"
elif [[ -f "$DATA_DIR/docs_tree.json" ]]; then
    DOCS_TREE="$DATA_DIR/docs_tree.json"
else
    print_error "Documentation tree not found in: $DATA_DIR"
...
if [[ "$SKIP_GENERATION" == false ]]; then
    ...
    while [[ -n "$remaining_models" ]]; do
        ...
        gen_cmd="python rubrics_generator/generate_rubrics.py --repo-name \"$REPO_NAME\" --model \"$model\" --data-dir \"$DATA_DIR\""
...
if [[ "$SKIP_COMBINATION" == false ]]; then
    ...
    COMBINE_MODEL="${MODELS%%,*}"
    combine_cmd="python rubrics_generator/combine_rubrics.py --repo-name \"$REPO_NAME\" --model \"$COMBINE_MODEL\""

    if [[ -n "$DATA_DIR" ]]; then
        combine_cmd="$combine_cmd --data-dir \"$DATA_DIR\""
    fi
