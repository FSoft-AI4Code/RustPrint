...
async def read_code_components(ctx: RunContext[RustPrintDeps], component_ids: list[str]) -> str:
    ...
    logger.info("=" * 80)
    logger.info("TOOL CALL: read_code_components")
    logger.info("  Count : %d component(s)", len(component_ids))
    for comp_id in component_ids:
        logger.info("  Component : %s", comp_id)
    logger.info("=" * 80)
