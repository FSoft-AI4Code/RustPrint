...
from rustprint.src.dependency_analyzer.utils.patterns import DEFAULT_IGNORE_PATTERNS, DEFAULT_INCLUDE_PATTERNS
...
class RepoAnalyzer:
    ...
    def _build_file_tree(self, repo_dir: str) -> Dict:
        def build_tree(path: Path, base_path: Path) -> Optional[Dict]:
            ...
            # 🚫 Reject symlinks (except the repository root itself)
            if path != base_path and path.is_symlink():
                return None
