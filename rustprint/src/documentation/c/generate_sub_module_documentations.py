...
from rustprint.src.prompt_template import SYSTEM_PROMPT, LEAF_SYSTEM_PROMPT, format_user_prompt
from rustprint.src.utils import is_complex_module, count_tokens
from rustprint.src.documentation.c.cluster_modules import format_potential_core_components
from rustprint.src.config import MAX_TOKEN_PER_LEAF_MODULE
...
async def generate_sub_module_documentation(
    ...
    model = create_main_model(deps.config)
    ...
    for sub_module_name, core_component_ids in sub_module_specs.items():
        ...
        if is_complex_module(ctx.deps.components, core_component_ids) and ctx.deps.current_depth < ctx.deps.max_depth and num_tokens >= MAX_TOKEN_PER_LEAF_MODULE:
            sub_agent = Agent(
                ...
                tools=[read_code_components_tool, str_replace_editor_tool, find_code_component_tool, generate_sub_module_documentation_tool],
            )
        else:
            sub_agent = Agent(
                model=model,
