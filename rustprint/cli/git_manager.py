"""
Git operations manager for RustPrint CLI.
"""
...
class GitManager:
        ...
        if not force:
            ...
            if not is_clean:
                raise RepositoryError(
                    ...
                    "  # or\n"
                    "  git stash\n\n"
                    "Then re-run: rustprint generate --create-branch"
                )
        
        # Generate branch name with timestamp
        timestamp = datetime.now().strftime("%Y%m%d-%H%M%S")
        branch_name = f"docs/rustprint-{timestamp}"
    ...
    def create_or_checkout_branch(self, branch_name: str) -> str:
        """
        Create a branch if needed, then check it out.

        Args:
            branch_name: Branch name to create or checkout

        Returns:
            The active branch name

        Raises:
            RepositoryError: If branch creation or checkout fails
        """
        try:
            ...
            raise RepositoryError(f"Failed to create or checkout branch '{branch_name}': {e}")
